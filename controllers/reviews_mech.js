                ...
                await axios.get(url).then(res => {
                    ...
                    $('.review-listing ').each(function (i, element) {
                        ...
                        const $reviewStore = $element
                            .find('div div .review-listing-header .review-listing-header__text')
                            .text()
                            .trim();
...
// POST Req
exports.retrieveNewestReviewsAndUpdateDb = asyncHandeler(async (req, res, next) => {
    // Get all app names
    const apps = await Apps.find();
        ...
        try {
            ...
            await axios.get(url).then(res => {
                ...
                $('.review-listing ').each(function (i, element) {
                    ...
                    //Check if Review Exists in DB
                    Review.exists({ storeName: review.storeName }, function (err, result) {
                        if (err) {
                            res.send(err);
                        } else if (result === null) {
                            //If no write it
...
exports.getNumberOfReviewsByStarRating = asyncHandeler(async (req, res, next) => {
...
exports.testRouteForScraper = asyncHandeler(async (req, res, next) => {
    ...
    await axios.get(url).then(res => {
        ...
        $('.review-listing ').each(function (i, element) {
            const $element = $(element);
            // console.log(element);
            const $reviewScore = $element.find('div div div div div div').attr('data-rating');
...
exports.getThisMonthLastMonth = asyncHandeler(async (req, res, next) => {
    ...
    // Last Month
    const startDayOfPrevWeek = moment(todayDate).subtract(1, 'month').startOf('month');
    const lastDayOfPrevWeek = moment(todayDate).subtract(1, 'month').endOf('month');
...
exports.getLast12Months = asyncHandeler(async (req, res, next) => {
    ...
    // Last Month
    const seventhStart = moment(todayDate).subtract(7, 'month').startOf('month');
    ...
    // Last Month
    const elevenStart = moment(todayDate).subtract(11, 'month').startOf('month');
